...
export const getFenceDetailsHtml = (FenceTypeselectedValue, NumberofSticks, inputValuePostspace, PostSpaceUnitselectedValue, displayValues) => `
  <!DOCTYPE html>
  <html lang="en">
  <head>
      <meta charset="UTF-8">
  ...
  <body>
      <div class="container">
          ...
          <ul>
          <li>Fence Typpe = ${FenceTypeselectedValue}</li>
          
          <li> Total Stick Amount = ${NumberofSticks}</li>
          <li> Post Space = ${inputValuePostspace} ${PostSpaceUnitselectedValue}</li>
          <li> Gate values  <ul>
          ${displayValues.map((value) => `<li>${value}</li>`).join("")}
          </ul></li>
